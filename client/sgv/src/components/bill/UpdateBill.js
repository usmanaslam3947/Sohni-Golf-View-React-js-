...
export default function UpdateBill(props) {
    ...
    const [billName,setBillName] = useState(props.object.state.bill.type);
    ...
    const billToUpdate = async(e) => {
        ...
        const response = await updateBill(billId,billName,billAmount,billCreatedDate,billDueDate);
    ...
    return(
        <div className="create">
            ...
            <form onSubmit={billToUpdate}>
                ...
                <button className="btn btn-secondary mt-1" onClick={()=>props.object.setState({update:false,display:true})}>Close</button>
            </form>
        </div>
    )
